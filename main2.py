...
transcript_files = ['transcript_4.txt']
key_score = []
final_score = []
...
for i in range(n):
    final_score.append(key_score[i])
...
if len(text) == 0:
    print("No keyword with score greater than 1 hence the documents are not similar")
...
if len(text) == 0:
    engine.say("No keyword with score greater than 1 hence the documents are not similar")
    engine.runAndWait()
